...
describe('HighlightMatch', () => {
  it('renders text without query', () => {
    render(<HighlightMatch text="Test text" query="" />);

    expect(screen.getByText('Test text')).toBeInTheDocument();
  });

  it('renders text without highlight when no matches', () => {
    render(<HighlightMatch text="Test text" query="xyz" />);

    expect(screen.getByText('Test text')).toBeInTheDocument();
  });

  it('highlights matching text', () => {
    const { container } = render(<HighlightMatch text="Test text" query="test" />);
  ...
  it('highlights multiple occurrences', () => {
    const { container } = render(<HighlightMatch text="test test test" query="test" />);
  ...
  it('applies custom highlight class name', () => {
    const { container } = render(
      <HighlightMatch text="Test text" query="test" highlightClassName="custom-highlight" />,
    );
  ...
  it('preserves non-highlighted text', () => {
    const { container } = render(<HighlightMatch text="The test string" query="test" />);

    expect(container.textContent).toBe('The test string');
  });

  it('handles partial word matching', () => {
    const { container } = render(<HighlightMatch text="Testing tested" query="test" />);
